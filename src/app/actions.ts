...
export async function submitLead(
  ...
  const { firstname, lastname, email, phone, message, montant, banque, situation, source } =
    parsed.data;

  const details = [
    montant && `Capital emprunté : ${montant}`,
    banque && `Banque prêteuse : ${banque}`,
